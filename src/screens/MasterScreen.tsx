...
export default function MasterScreen({ master, onUpdate, onBack }: Props) {
  ...
  function deleteItem(ci: number, ii: number) {
    ...
    save(updated);
    saveTrash([...trashItems, { item, fromCategory: catName, deletedAt: new Date().toISOString() }], trashCats);
  ...
  return (
    <div className="min-h-dvh bg-slate-50 pb-10">
      ...
      {/* Tab bar */}
      <div className="sticky top-[61px] z-40 bg-slate-50 border-b border-slate-100 px-4 py-2.5 flex gap-2">
      ...
      <div className="max-w-lg mx-auto px-4 py-3">
        {!showTrash ? (
          <>
                  ...
                  <div key={ci} className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
                    <div
                      className="flex items-center gap-3 py-3.5 px-4 cursor-pointer
                        hover:bg-slate-50/60 transition-colors select-none"
                    ...
                    >
                      ...
                      <div className="flex-1 font-bold text-[14px]" style={{ color: cat.color }}>
                        {cat.name}
                      </div>
                      <span
                        className="text-[11px] py-0.5 px-2.5 rounded-full font-semibold"
                        style={{ background: cat.color + '18', color: cat.color }}
                      >
                        {cat.items.length}
                      </span>
                      <button
                        onClick={e => { e.stopPropagation(); deleteCategory(ci); }}
                        className="w-7 h-7 rounded-lg hover:bg-red-50 text-slate-300 hover:text-red-400
                          flex items-center justify-center cursor-pointer transition-all border-none bg-transparent"
                      >
                        <Trash2 size={14} />
                      </button>
                      {isCol
                    ...
                    {!isCol && (
                      <div className="px-3 pb-3 border-t border-slate-50">
                        {cat.items.map((item, ii) => (
                          <div key={ii} className="flex items-center gap-2.5 py-2 px-1.5 rounded-xl
                            hover:bg-slate-50 transition-colors">
                            <div className="flex-1 text-sm text-slate-800">{item.name}</div>
                            <FreqTag freq={item.freq} />
                            <button
                              onClick={() => { setEditItem({ ci, ii }); setEditName(item.name); setEditFreq(item.freq); }}
                              className="w-7 h-7 rounded-lg hover:bg-indigo-50 text-slate-300 hover:text-indigo-500
                                flex items-center justify-center cursor-pointer transition-all border-none bg-transparent"
                            >
                              <Pencil size={13} />
                            </button>
                        ...
                        <button
                          onClick={() => { setAddItemCat(ci); setNewItemName(''); setNewItemFreq('monthly'); }}
                          className="mt-1.5 border-2 border-dashed border-slate-200 rounded-xl py-2 px-3.5
                            w-full bg-transparent text-[12px] text-slate-400 cursor-pointer font-semibold
                            hover:border-indigo-300 hover:text-indigo-400 transition-all
            ...
            <button
              onClick={() => { setAddCatOpen(true); setNewCatName(''); setNewCatIcon('📦'); setNewCatColor('#607d8b'); }}
              className="mt-3 border-2 border-dashed border-slate-200 rounded-2xl py-4 w-full
                bg-transparent text-sm text-slate-400 cursor-pointer font-bold
                hover:border-indigo-300 hover:text-indigo-400 transition-all
          ...
          <>
                  ...
                  {trashCats.map((tc, i) => (
                    <div key={i} className="bg-white rounded-2xl border border-slate-100 p-4">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => restoreCategory(tc)}
                          className="flex-1 py-2.5 border-none rounded-xl font-bold text-xs cursor-pointer
                            bg-emerald-50 text-emerald-600 flex items-center justify-center gap-1.5
                            active:scale-95 transition-all"
                          style={{ fontFamily: 'inherit' }}
                        >
                          <RotateCcw size={13} /> שחזר
                        </button>
                        <button
                          onClick={() => permanentDeleteCat(tc)}
                          className="flex-1 py-2.5 border-none rounded-xl font-bold text-xs cursor-pointer
                            bg-red-50 text-red-500 flex items-center justify-center gap-1.5
                            active:scale-95 transition-all"
                          style={{ fontFamily: 'inherit' }}
                        >
                ...
                <div className="flex flex-col gap-1.5">
                  {trashItems.map((ti, i) => (
                    <div key={i} className="bg-white rounded-xl border border-slate-100 py-3 px-4 flex items-center gap-3">
                      ...
                      <button
                        onClick={() => { setRestoreItem(ti); setRestoreCatIdx(0); }}
                        className="border-none bg-emerald-50 text-emerald-600 py-1.5 px-3 rounded-lg
                          text-[11px] font-semibold cursor-pointer flex items-center gap-1
                          active:scale-95 transition-all"
                        style={{ fontFamily: 'inherit' }}
                      >
                      ...
                      <button
                        onClick={() => permanentDeleteItem(ti)}
                        className="w-7 h-7 rounded-lg hover:bg-red-50 text-slate-300 hover:text-red-400
                          flex items-center justify-center cursor-pointer transition-all border-none bg-transparent"
                      >
                        <X size={14} />
            ...
            {!trashItems.length && !trashCats.length && (
              <div className="text-center py-14">
                ...
                <p className="font-semibold text-slate-400 text-sm">האשפה ריקה</p>
              </div>
            )}
          </>
        )}
        ...
        <div className="flex gap-2.5 mb-1">
          <button
            ...
            className="flex-1 py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
              bg-slate-100 text-slate-500 active:scale-95 transition-all"
            style={{ fontFamily: 'inherit' }}
          >
            ביטול
          </button>
          <button
            ...
            className="flex-1 py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
              bg-indigo-500 text-white active:scale-95 transition-all"
            style={{ fontFamily: 'inherit' }}
          >
            הוסף
          </button>
      ...
      <CenterModal open={addItemCat !== null} onClose={() => setAddItemCat(null)}>
        <h2 className="font-bold text-base mb-4 text-center text-slate-900">
          פריט חדש{addItemCat !== null ? ` ל${master[addItemCat]?.name}` : ''}
        </h2>
        <input
      ...
      <CenterModal open={restoreItem !== null} onClose={() => setRestoreItem(null)}>
        ...
        <p className="text-sm text-center text-slate-500 mb-4">
          לאיזה קטגוריה להחזיר את "<span className="font-bold text-slate-800">{restoreItem?.item.name}</span>"?
        </p>
        <div className="max-h-[40vh] overflow-auto mb-4 flex flex-col gap-1.5">
          {master.map((cat, ci) => (
            <div
              ...
              className={`flex items-center gap-3 py-2.5 px-3.5 rounded-xl cursor-pointer
