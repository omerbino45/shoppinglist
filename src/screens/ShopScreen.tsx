...
import {
  ...
  Copy, RotateCcw, Copy as CopyIcon, ChevronDown, ChevronUp, Undo2,
} from 'lucide-react';
...
export default function ShopScreen({ list, master, allLists, onUpdate, onListsChange, onBack }: Props) {
  ...
  return (
    <div className="min-h-dvh bg-slate-50 pb-10">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-1.5 bg-white/20 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${pct}%`, background: pct === 100 ? '#6ee7b7' : 'white' }}
            />
          </div>
          <span className="text-white/75 text-xs font-medium whitespace-nowrap">
            {done}/{total}
          </span>
          <span className="text-white/90 text-xs font-bold">{pct}%</span>
        </div>
      </Header>
          ...
          <div className="flex gap-1.5 mr-auto">
            <button
              ...
              className="py-1.5 px-3.5 rounded-full text-xs font-semibold cursor-pointer
                bg-indigo-500 text-white flex items-center gap-1 active:scale-95 transition-all"
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-500
                flex items-center justify-center cursor-pointer hover:border-slate-300
      ...
      {undo && (
        <div
          className="fixed bottom-5 left-1/2 -translate-x-1/2 bg-slate-900 text-white
        ...
        >
          ...
          <button
            onClick={handleUndo}
            className="bg-indigo-400 text-white border-none py-1.5 px-4 rounded-xl
              font-bold text-xs cursor-pointer flex items-center gap-1.5 active:scale-95 transition-all"
            style={{ fontFamily: 'inherit' }}
          >
            <Undo2 size={12} /> בטל
          </button>
      ...
      <BottomSheet open={editOpen} onClose={() => setEditOpen(false)}>
        ...
        <label className="text-[12px] font-semibold text-slate-400 block mb-1.5 uppercase tracking-wide">
          תאריך
        </label>
        <input
          type="date"
        ...
        <div className="flex flex-col gap-2.5">
          <button
            ...
            className="py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
              bg-slate-100 text-slate-700 flex items-center justify-center gap-2
              active:scale-95 transition-all"
            style={{ fontFamily: 'inherit' }}
          >
            <Copy size={16} /> העתק לוואצאפ
          </button>
          <button
            onClick={handleReset}
            className="py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
            ...
            style={{ fontFamily: 'inherit' }}
          >
            <RotateCcw size={16} /> איפוס סימונים
          </button>
          <button
            onClick={() => { setDupDate(''); setDupOpen(true); setEditOpen(false); }}
            className="py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
              bg-indigo-50 text-indigo-600 flex items-center justify-center gap-2
              active:scale-95 transition-all"
            style={{ fontFamily: 'inherit' }}
          >
            <CopyIcon size={16} /> שכפל רשימה
          </button>
          {!list.closed ? (
            <button
              onClick={handleClose}
              className="py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
                bg-red-50 text-red-500 flex items-center justify-center gap-2
                active:scale-95 transition-all"
              style={{ fontFamily: 'inherit' }}
            >
            ...
            <button
              ...
              className="py-3.5 border-none rounded-xl font-bold text-sm cursor-pointer
                bg-emerald-50 text-emerald-600 flex items-center justify-center gap-2
                active:scale-95 transition-all"
              style={{ fontFamily: 'inherit' }}
            >
              <LockOpen size={16} /> פתח מחדש
            </button>
          )}
        </div>
        <button
          onClick={() => setEditOpen(false)}
          className="mt-3.5 w-full py-3 border-none rounded-xl bg-slate-100 text-slate-400
            font-semibold text-sm cursor-pointer active:scale-95 transition-all"
          style={{ fontFamily: 'inherit' }}
        >
          סגור
        </button>
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full py-2.5 pr-10 pl-4 border border-slate-200 rounded-xl text-sm
              ...
              focus:ring-2 focus:ring-indigo-100 transition-all font-[inherit] placeholder:text-slate-400"
          />
        </div>
          ...
          {master.map((cat, ci) => {
            ...
            return (
              <div key={ci} className="mb-3">
                <div
                  className="text-[12px] font-bold mb-1.5 flex items-center gap-1.5"
                  ...
                  return (
                    ...
                    >
                      ...
                      {exists
                        ...
                        : <span className="text-[10px] py-1 px-2.5 rounded-full font-semibold bg-indigo-50 text-indigo-500 flex items-center gap-1">
                            <Plus size={9} /> הוסף
                          </span>
                      }
                    </div>
